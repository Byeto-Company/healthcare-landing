...
// props
    ...
    const { heroData, logo } = toRefs(props)
...
// computed
    ...
    const heroImages: {link: string, alt: string}[] = computed(() => {
        return heroData.value.images.map((item) => {
            return {
    ...
    const heroLogo: {link: string, alt: string} = computed(() => {
        return {
            link: `${config.public.API_BASE_URL}/${logo.value.link}`,
            alt: logo.value.alt
        }
    })
    ...
    <header class="relative container h-full padding-main flex-center max-md:flex-col-reverse w-full md:gap-10 xl:gap-16 lg:w-[90%] pt-8">
        ...
        <div class="flex flex-col items-center w-full gap-4 lg:items-start md:w-1/2 xl:gap-8">
            <div class="flex flex-col items-center lg:items-start">
                ...
                <span class="py-0.5 text-3xl xl:text-5xl !leading-[150%] font-bold text-gradient bg-gradient-to-r from-primary to-secondary">{{ heroData.title }}</span>
            </div>
        ...
        <div class="flex items-center justify-center w-full md:justify-end md:w-1/2">
            ...
            <div class="relative flex-center aspect-square size-[430px]">
                ...
                <div class="p-4 bg-white flex-center rounded-360 xl:mt-6">
                    <NuxtImg :src="heroLogo.link" :alt="heroLogo.alt" class="size-7 xl:size-10" />
                </div>
